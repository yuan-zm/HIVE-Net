...
def division_array(crop_size, crop_num1, crop_num2, crop_num3, ori_img_shape):
    """Make division array
    Args :
        crop_size(int) : size of cropped image
        crop_num2 (int) : number of crop in horizontal way
    ...
    """
    dim1, dim2, dim3 = ori_img_shape

    div_array = np.zeros([dim1, dim2, dim3])  # make division array
    one_array = np.ones([crop_size[0], crop_size[1], crop_size[2]])  # one array to be added to div_array

    dim1_stride = stride_size(dim1, crop_num1, crop_size[0])  # depth stride
    dim2_stride = stride_size(dim2, crop_num2, crop_size[1])  # vertical stride
    dim3_stride = stride_size(dim3, crop_num3, crop_size[2])  # horizontal stride
    for i in range(crop_num1):
        for j in range(crop_num2):
            for k in range(crop_num3):
                # add ones to div_array at specific position
                div_array[dim1_stride*i:dim1_stride*i + crop_size[0],
...
def image_concatenate(image, crop_n1, crop_n2, crop_n3, dim1, dim2, dim3):
    ...
    empty_array = np.zeros([dim1, dim2, dim3]).astype("float64")  # to make sure no overflow
    dim1_stride = stride_size(dim1, crop_n1, crop_size[0])  # depth stride
    dim2_stride = stride_size(dim2, crop_n2, crop_size[1])  # vertical stride
    dim3_stride = stride_size(dim3, crop_n3, crop_size[2])  # horizontal stride
    index = 0
        ...
        for j in range(crop_n2):
            for k in range(crop_n3):
                ...
                empty_array[dim1_stride*i:dim1_stride*i + crop_size[0],
                            dim2_stride*j:dim2_stride*j + crop_size[1],
                            dim3_stride*k:dim3_stride*k + crop_size[2]] += image[index]
                index += 1
    return empty_array
